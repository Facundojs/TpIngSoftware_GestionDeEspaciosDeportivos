            ...
            using (var uow = DalFactory.CreateUnitOfWork())
            {
                try
                {
                    ...
                    if (dto.ReservaID.HasValue)
                    {
                        ...
                        if (reserva == null) throw new InvalidOperationException(Domain.Enums.Translations.ERR_RESERVA_NO_EXISTE.Translate());

                        var espacio = uow.EspacioRepository.GetById(reserva.EspacioID);
                        ...
                        decimal pagadoHastaAhora = pagosAnteriores
                            .Where(p => p.Estado == EstadoPago.Abonado.ToString())
                            .Sum(p => p.Monto);
                        ...
                        {
                            throw new InvalidOperationException(Domain.Enums.Translations.ERR_MONTO_SUPERA_SALDO.Translate());
                        }
                    }
                    ...
                    if (reserva != null)
                    {
                        var pagosActuales = uow.PagoRepository.GetByReserva(reserva.Id);
                        decimal pagadoTotal = pagosActuales
                            .Where(p => p.Estado == EstadoPago.Abonado.ToString())
                            .Sum(p => p.Monto);
                    ...
                    try
                    {
                        var savedPago = _pagoRepo.GetById(pagoEntity.Id);
                    ...
                    {
                        _bitacora.Log($"CU-PA-001: Receipt generation failed: {logEx.Message}", "INFO");
                    }
                }
                catch (Exception ex)
                {
        ...
        public void ReembolsarPago(Guid pagoId)
        {
            using (var uow = DalFactory.CreateUnitOfWork())
            {
                try
                {
                    ...
                    if (pago == null) throw new InvalidOperationException(Domain.Enums.Translations.ERR_PAGO_NO_EXISTE.Translate());

                    if (pago.Estado != EstadoPago.Abonado.ToString())
                    {
                        throw new InvalidOperationException(Domain.Enums.Translations.ERR_SOLO_ABONADO_ESTADO.Translate());
                    }
                    ...
                    var cliente = _clienteRepo.GetById(pago.ClienteID);
                    _bitacora.Log($"CU-PA-004: Payment #{pago.Codigo} refunded - Client {cliente?.DNI}", "INFO");
                }
                catch (Exception ex)
                {
                    uow.Rollback();
